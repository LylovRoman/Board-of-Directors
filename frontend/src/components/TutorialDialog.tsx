...
export function TutorialDialog(props: { onClose: () => void }) {
  const steps = [
    ...
    { title: "3. Выбери меморандум", text: "Обычный Директор выбирает стартовую подсказку про возможности или риски. Комплаенс выбирает предпочтение для усиленного стартового меморандума; если не успеет, получит возможности по умолчанию." },
    ...
    { title: "8. Усиленный меморандум", text: "Обычные Директора получают прежнюю тройку. Комплаенс получает усиленную пару со старта: в рисках есть цель Крота, в возможностях есть чистое решение." },
  ];
